...
class DT:
    def __init__(self) -> None:
        ...
        self.DTree = RandomForestClassifier(n_estimators=100,random_state=373)
    ...
    def showAccuracy(self):
        Y_predict = self.DTree.predict(self.X_test)# X_test 的預測結果
        # 計算混淆矩陣
        cm = confusion_matrix(self.Y_test, Y_predict)
        print('Confusion Matrix:')
        print(cm)
        return f"Test score: {accuracy_score(self.Y_test, Y_predict)}"
    ...
    def Predict(self,URL):
        ...
        Pre_ans = self.DTree.predict(predict_data) # the predict answer
        pre = [Pre_ans[0]]#nparray to array
        output = ''
        output += f"\n該網址是 : {'正常網站' if pre[0] == 'benign' else '釣魚網站'}" 
        return output
